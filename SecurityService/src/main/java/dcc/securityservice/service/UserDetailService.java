package dcc.securityservice.service;
...
public class UserDetailService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String combinedUsername) throws UsernameNotFoundException {
        ...
        if (userType.equals("Enseignant")){
            System.out.println("YES Prof : "+email);
            user_infos = restTemplate.getForObject("http://localhost:8081/v1/enseignants/email/"+email,HashMap.class);

        }

        if (userType.equals("Chercheur")){

            user_infos = restTemplate.getForObject("http://localhost:8082/v1/chercheurs/email/"+email,HashMap.class);
        }
        ...
        return new User(user_infos.get("email"), user_infos.get("password"), authorities);
    }
}
